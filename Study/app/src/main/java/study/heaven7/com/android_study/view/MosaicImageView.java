...
public class MosaicImageView extends View {
    ...
    private int mMosaicColor = Color.BLACK;
    /**
     * 正在处理马赛克
     */
    private boolean mDoingMosaic = true;

    public MosaicImageView(Context activity, AttributeSet attrs, Bitmap bitmap) {
        ...
        mPaint.setStrokeWidth(PAINT_STROKEWIDTH / 2);
        mPath = new Path();

        bitmap.recycle();

        setColorBlock();
      //TODO  getContext().sendBroadcast(new Intent(EditUpcaseActivity.ACTION_CANCEL_DIALOG));
    ...
    public void setDoingMosaic(boolean doingMosaic) {
        this.mDoingMosaic = doingMosaic;
    }
    public void toogleMosaicState() {
        this.mDoingMosaic = !mDoingMosaic;
    }
    //取色块值则不需要
    public void setMosaicColor(int mosaicColor) {
     ...
     * 恢复
     */
    public void recover(Bitmap bitmap) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_DOWN:
                if (mDoingMosaic && event.getPointerCount() == 1) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mDoingMosaic && event.getPointerCount() == 1) {
                    ...
                    double fingerDis = distanceBetweenFingers(event); //手指之间的距离
    ...
    private void touch_move(float x, float y) {
        ...
        if (dx >= TOUCH_TOLERANCE || dy >= TOUCH_TOLERANCE) {
            ...
            mX = x;
            mY = y;
           //TODO getContext().sendBroadcast(new Intent(EditUpcaseActivity.ACTION_EDITED));
        }
    }
    ...
    public void drawStrokeSize(Canvas canvas) {
        ...
        paint.setAntiAlias(true);
        //绘制绘制指示点原形边框
        paint.setColor(getResources().getColor(R.color.c_fc6f51));
        paint.setStyle(Style.STROKE);
        paint.setStrokeWidth(4f);
        if (totalRatio > 1) {
    ...
    public void setStrokeMultiples2(float ratio) {
        strokeMultiples = totalRatio * ratio;
        mPaint.setStrokeWidth(PAINT_STROKEWIDTH / totalRatio * strokeMultiples);
        ViewCompat.postInvalidateOnAnimation(this);
    }
    ...
    public static Bitmap getImage(String srcPath, int newWidth, int newHeight) {
        // Bitmap bitmap = ImageUtil.getLoacalBitmap(getContext(), srcPath);
        Bitmap bitmap = new ImageParser(newWidth, newHeight).parseToBitmap(srcPath);
